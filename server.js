...
// Fix yahoo-finance2 validation
try { yahooFinance.setGlobalConfig({ validation: { logErrors: false, logOptionsErrors: false } }); } catch(e) {}
...
async function fetchYahoo(symbol, tf) {
  ...
  // Fix: suppress validation errors
  const result=await yahooFinance.chart(ySym,{interval,range,includePrePost:false},{validateResult:false});
  const quotes=(result.quotes||[]).filter(q=>q.open&&q.close);
  if (quotes.length<30) throw new Error(`Not enough data for ${symbol}. Try a different timeframe.`);
