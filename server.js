...
async function fetchYahooCrypto(symbol, tf) {
  // Use Yahoo Finance for crypto (BTC-USD format) — works from any location
  const m={"5m":{interval:"5m",range:"5d"},"15m":{interval:"15m",range:"5d"},"1h":{interval:"60m",range:"1mo"},"4h":{interval:"1d",range:"3mo"},"1d":{interval:"1d",range:"1y"},"1w":{interval:"1wk",range:"2y"}};
  const {interval,range}=m[tf]||m["1d"];
  const clean=symbol.toUpperCase().replace("USDT","").replace("-USD","");
  const ySym=clean+"-USD"; // BTC-USD, ETH-USD etc
  ...
  const quotes=(result.quotes||[]).filter(q=>q.open&&q.close);
  if (quotes.length<30) throw new Error(`Not enough crypto data for ${symbol}`);
  return {opens:quotes.map(q=>q.open),highs:quotes.map(q=>q.high),lows:quotes.map(q=>q.low),closes:quotes.map(q=>q.close),volumes:quotes.map(q=>q.volume||0),livePrice:quotes[quotes.length-1].close,source:"yahoo_crypto"};
}
...
async function getMarketData(symbol, tf) {
  ...
  try {
    if (market==="CRYPTO") {
      // Try Binance first, fallback to Yahoo Finance crypto
      try {
        const clean=symbol.toUpperCase().replace("-USD","").replace("USDT","");
        data=await binanceEngine.getOHLCV(clean,tf);
      } catch(e) {
        console.warn(`Binance failed [${symbol}]: ${e.message} → Yahoo crypto`);
        data=await fetchYahooCrypto(symbol,tf);
      }
    ...
    console.warn(`All sources failed [${symbol}]: ${e.message}`);
