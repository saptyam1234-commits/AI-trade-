// ═══════════════════════════════════════════════════════════════
//  TRADESIGNAL AI — SMC SERVER v3.2 (SINGLE FILE)
//  All modules merged — RR Waterfall (OB → FVG → Premium/Discount)
// ═══════════════════════════════════════════════════════════════
...
class SMCEngine {
  ...
  static detectBOS(highs, lows, closes) {
    ...
    if (swingHighs.length >= 2) {
      ...
      if (cur > l.price && l.price > p.price) results.push({ type: "BOS_BULLISH", level: l.price, description: `Bullish BOS above ${l.price.toFixed(2)}`, strength: "HIGH" });
    }
    if (swingLows.length >= 2) {
      const l = swingLows[swingLows.length - 1], p = swingLows[swingLows.length - 2];
      if (cur < l.price && l.price < p.price) results.push({ type: "BOS_BEARISH", level: l.price, description: `Bearish BOS below ${l.price.toFixed(2)}`, strength: "HIGH" });
    }
    return results;
  }
  ...
  static detectCHoCH(highs, lows, closes) {
    ...
    if (swingHighs.length >= 1 && swingLows.length >= 2) {
      const sh = swingHighs[swingHighs.length - 1], ll = swingLows[swingLows.length - 1], pl = swingLows[swingLows.length - 2];
      if (ll.price < pl.price && cur > sh.price) results.push({ type: "CHOCH_BULLISH", level: sh.price, description: `Bullish CHoCH at ${sh.price.toFixed(2)}`, strength: "VERY_HIGH" });
    }
    if (swingLows.length >= 1 && swingHighs.length >= 2) {
      const sl = swingLows[swingLows.length - 1], lh = swingHighs[swingHighs.length - 1], ph = swingHighs[swingHighs.length - 2];
      if (lh.price > ph.price && cur < sl.price) results.push({ type: "CHOCH_BEARISH", level: sl.price, description: `Bearish CHoCH at ${sl.price.toFixed(2)}`, strength: "VERY_HIGH" });
    }
    return results;
  }
  ...
  static detectOrderBlocks(opens, highs, lows, closes, volumes) {
    ...
    for (let i = 3; i < closes.length - 2; i++) {
      ...
      if (closes[i] < opens[i] && closes[i+1] > opens[i+1] && closes[i+2] > closes[i+1] && hv)
        obs.push({ type: "OB_BULLISH", top: opens[i],  bottom: closes[i], index: i, description: `Bullish OB ${closes[i].toFixed(2)}–${opens[i].toFixed(2)}`,  strength: str ? "STRONG" : "NORMAL" });
      if (closes[i] > opens[i] && closes[i+1] < opens[i+1] && closes[i+2] < closes[i+1] && hv)
        obs.push({ type: "OB_BEARISH", top: closes[i], bottom: opens[i],  index: i, description: `Bearish OB ${opens[i].toFixed(2)}–${closes[i].toFixed(2)}`, strength: str ? "STRONG" : "NORMAL" });
    }
    return obs.slice(-3);
  }
  ...
  static calcRiskManagement(entryPrice, stopLoss, targetPrice, accountSize, riskPercent = 1) {
    ...
    return {
      ...
      potentialProfit: parseFloat((positionSize * rewardPerUnit).toFixed(2)),
      rrRatio,
      riskPercent,
  ...
  // ─────────────────────────────────────────────────────────
  //  RR WATERFALL: AI → OB → FVG → Premium/Discount → Reject
  // ─────────────────────────────────────────────────────────
  static findBestEntry(smcData, aiSignal, accountSize) {
    const isBull = smcData.overallBias === "BULLISH";
    const sl     = parseFloat(aiSignal.stopLoss) || smcData.suggestedSL;
    const target = parseFloat(aiSignal.target1)  || smcData.suggestedTarget;
    const price  = smcData.currentPrice;

    // Helper — build a full risk record for a given entry
    const tryEntry = (entry, label) => {
      if (!entry || isNaN(entry)) return null;
      const risk = this.calcRiskManagement(entry, sl, target, accountSize, 1);
      if (!risk) return null;
    ...
    // Step 1 — AI suggested entry (baseline)
    candidates.push(tryEntry(parseFloat(aiSignal.entry) || price, "AI_SUGGESTED"));

    // Step 2 — Order Block entry (tightest risk, best RR)
    if (smcData.orderBlocks?.length > 0) {
      ...
      if (ob) {
        const obEntry = isBull ? ob.bottom : ob.top;
        candidates.push(tryEntry(obEntry, "OB_ENTRY"));
      }
    }

    // Step 3 — FVG midpoint entry
    if (smcData.fvg?.length > 0) {
      ...
      if (fvg) {
        ...
        candidates.push(tryEntry(fvgMid, "FVG_MID"));
      }
    }

    // Step 4 — Premium / Discount zone entry (25% into zone from support/resistance)
    const hi  = smcData.keyLevels?.immediateResistance || price * 1.02;
    const lo  = smcData.keyLevels?.immediateSupport    || price * 0.98;
    const eq  = (hi + lo) / 2;
    const discountEntry = lo + (eq - lo) * 0.25;   // deep discount for longs
    const premiumEntry  = hi - (hi - eq) * 0.25;   // deep premium for shorts
    candidates.push(tryEntry(isBull ? discountEntry : premiumEntry, "PREMIUM_DISCOUNT"));

    // Pick first candidate with RR >= 2, prefer highest RR
    ...
    // All failed — reject the trade
    return {
      accepted:      false,
      reason:        "RR < 2 on all entry types (AI, OB, FVG, Premium/Discount). Trade rejected.",
      bestAvailable: candidates.filter(Boolean).sort((a, b) => b.rrRatio - a.rrRatio)[0] || null,
      triedEntries:  candidates,
    };
  }
  ...
  static fullAnalysis(data) {
    ...
    ms.forEach(s => {
      if (s.type === "HH") bullScore += 15;
      if (s.type === "HL") bullScore += 20;
      if (s.type === "LL") bearScore += 15;
      if (s.type === "LH") bearScore += 20;
    });
...
class BinanceEngine extends EventEmitter {
  ...
  fetchKlines(symbol, tf, limit = 300) {
    return new Promise((resolve, reject) => {
      ...
      const req  = https.request({ hostname:"api.binance.com", path, method:"GET", headers:{"User-Agent":"TradeSignalAI/3.2"} }, res => {
        let d = ""; res.on("data", c => d += c); res.on("end", () => {
          try {
            const r = JSON.parse(d);
            if (!Array.isArray(r)) return reject(new Error(`Binance: ${JSON.stringify(r)}`));
            resolve(r.map(k => ({ time:k[0], open:parseFloat(k[1]), high:parseFloat(k[2]), low:parseFloat(k[3]), close:parseFloat(k[4]), volume:parseFloat(k[5]) })));
          } catch(e) { reject(e); }
        });
      });
      req.on("error", reject);
    ...
    ws.on("message", raw => {
      try {
        ...
        if (this.candles[key]?.length > 0) {
          ...
          if (arr[arr.length-1].time === candle.time) arr[arr.length-1] = candle;
          else if (candle.closed) { arr.push(candle); if (arr.length > 500) arr.shift(); }
        }
  ...
  async getOHLCV(symbol, tf) {
    ...
    if (this.candles[key]?.length >= 50) {
      const arr = this.candles[key];
      return { opens:arr.map(c=>c.open), highs:arr.map(c=>c.high), lows:arr.map(c=>c.low), closes:arr.map(c=>c.close), volumes:arr.map(c=>c.volume), livePrice:this.ticker[symbol]||arr[arr.length-1].close, source:"binance_ws" };
    }
    const candles = await this.fetchKlines(symbol, tf);
    this.candles[key] = candles; this.ticker[symbol] = candles[candles.length-1].close;
...
function httpGetJson(hostname, path) {
  return new Promise((resolve, reject) => {
    const req = https.request({ hostname, path, method:"GET", headers:{"User-Agent":"TradeSignalAI/3.2","Accept":"application/json"} }, res => {
...
async function fetchCoinGecko(symbol, tf) {
  ...
  const raw  = await httpGetJson("api.coingecko.com", `/api/v3/coins/${coinId}/ohlc?vs_currency=usd&days=${days}`);
...
function buildLayer1(smcData) {
  ...
  return {
    bias: overallBias, engineScore: confidence, confluences, confluenceCount: confluences.length,
    currentPrice, ema: emaData, msTrend: ms.trend || "UNDEFINED", msItems: ms.structure || [],
    bos, choch, orderBlocks, liquidity, fvg, slHunts, volume: volumeData, keyLevels,
    suggestedEntry, suggestedSL, suggestedTarget,
    hasOB: orderBlocks.length > 0, hasLiq: liquidity.length > 0, hasChoch: choch.length > 0,
  };
}
...
function parseDecision(text, source, layer1) {
  try {
    ...
    p.aiSource = source; p.layer1 = layer1; return p;
  } catch {
    return {
      ...
      entry: String(layer1.suggestedEntry), stopLoss: String(layer1.suggestedSL), target1: String(layer1.suggestedTarget),
      target2: "—", rrRatio: "—", confluenceScore: Math.min(10, layer1.confluenceCount),
      ...
      verdictReason: "Fallback — Gemini response unparseable", aiSource: source, layer1,
    };
  }
}
...
function formatSignal(symbol, tf, decision, smcData, riskData) {
  ...
  if (riskData) msg += `\n━━━━━━━━━━━━━━━━━━━━\n💼 *RISK(1%)*: Size:${riskData.positionSize} | Profit:$${riskData.potentialProfit} | RR:${riskData.rrRatio} | Valid:${riskData.isValidSetup?"✅":"❌"}`;
...
app.get("/", (req, res) => res.json({
  status: "TradeSignal AI — SMC Server v3.2 ✅",
  rrWaterfall: ["AI_SUGGESTED", "OB_ENTRY", "FVG_MID", "PREMIUM_DISCOUNT", "REJECT_IF_ALL_FAIL"],
  dataSources:  { crypto:"CoinGecko (FREE)", forex:"Twelve Data (FREE)", indian:"Yahoo Finance (FREE)" },
  features:     ["SMC","BOS/CHoCH","HH/HL/LL/LH","OB","Liquidity","FVG","SL Hunt","EMA 5/10/20/30","RR Waterfall","Gemini","Telegram"],
}));
...
app.post("/analyze", async (req, res) => {
  const {
    symbol, timeframe = "1h", geminiKey, telegramToken, telegramChannel,
    accountSize = 10000, extraContext = "", sendToTelegram = false,
  } = req.body;
  ...
  try {
    ...
    if (!entryResult.accepted) {
      // Trade rejected — RR < 2 on all candidates
      return res.json({
        success:       false,
        rejected:      true,
        ...
        triedEntries:  entryResult.triedEntries,
        bestAvailable: entryResult.bestAvailable,
        smc: {
    ...
    // Override AI entry with best valid RR entry
    ...
    if (sendToTelegram && telegramToken && telegramChannel && aiSignal.confidence >= 65) {
      try {
        await sendTelegram(telegramToken, telegramChannel, formatSignal(symbol, timeframe, aiSignal, smcData, riskData));
        telegramSent = true;
    ...
    res.json({
      ...
      dataSource:   marketData.source,
      market:       { market: classifyMarket(symbol) },
      entryLabel:   entryResult.label,
      smc: {
        bias:            smcData.overallBias,
        confidence:      smcData.confidence,
        ema:             smcData.emaData,
...
app.post("/scan", async (req, res) => {
  const {
    symbols, timeframe = "1h", geminiKey, telegramToken, telegramChannel,
    minConfidence = 65, accountSize = 10000, sendToTelegram = false,
  } = req.body;
  ...
  for (const symbol of symbols) {
    try {
      ...
      results.push({
        ...
        target1:         ai.target1,
        target2:         ai.target2,
        rrRatio:         entryResult.rrRatio,
        setupQuality:    ai.setupQuality,
        msTrend:         smc.marketStructure?.trend,
        emaTrend:        smc.emaData?.trend,
        aiSource:        ai.aiSource,
        riskValid:       entryResult.isValidSetup,
        bestTimeToEnter: ai.bestTimeToEnter,
      });

      if (ai.confidence >= minConfidence && ["BUY","SELL"].includes(ai.signal)) {
        highConf.push({ symbol, timeframe, signal: ai, smcData: smc, riskData: entryResult });
      }
  ...
  if (sendToTelegram && telegramToken && telegramChannel && highConf.length > 0) {
    for (const s of highConf.slice(0, 5)) {
      try {
        await sendTelegram(telegramToken, telegramChannel, formatSignal(s.symbol, s.timeframe, s.signal, s.smcData, s.riskData));
...
app.post("/telegram/test", async (req, res) => {
  ...
  try {
    ...
    await bot.sendMessage(telegramChannel, "✅ *TradeSignal AI SMC v3.2 Connected!* 🚀\n_RR Waterfall: AI → OB → FVG → Premium/Discount_", { parse_mode:"Markdown" });
...
// ═══════════════════════════════════════════════════════
//  AUTO SCAN CRON (every 4H Mon–Fri)
// ═══════════════════════════════════════════════════════
cron.schedule("0 9,13,17,21 * * 1-5", async () => {
  ...
  for (const symbol of watchlist) {
    try {
      ...
      results.push({ symbol, signal:ai.signal, verdict:ai.verdict, confidence:ai.confidence, entry:ai.entry, stopLoss:ai.stopLoss, target1:ai.target1, accepted:entryResult.accepted });
  ...
  console.log(`✅ Auto scan done. ${results.length} scanned. ${results.filter(r=>r.accepted).length} valid RR.`);
});
